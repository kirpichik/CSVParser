...
class CSVParser {
 ...
 private:
   ...
   */
  std::string readColoumnWithEscapeing(std::istream& stream,
  ...
  auto readColoumn(std::istream& stream, pos_type<N>, size_t& position) const
      throw(CSVParseException) {
   ...
   */
  template <typename tuple, size_t N>
  auto readColoumn(std::istream& stream, pos_type<1>, size_t& position) const
      throw(CSVParseException) {
